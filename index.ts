...
import {
    ...
    fillVerticalCracks,
    getRawShapes,
    largestShape, RawShape
} from "./geom.js";
import {
    advanceState,
    detectBody,
    extractFaceInfo, getNeck,
    guessApproximateHeadWidth, motionBit,
    MotionStats,
    preprocessShapes, refineHeadThresholdData, refineThresholdData, rotate90,
    thresholdBit
} from "./body-detection.js";
import {drawBackgroundImage, drawFace, drawHistogram, drawRawShapes, drawShapes} from "./debug-drawing.js";

const minFrame: number = -1;//256;//151;
const maxFrame: number = -1;//315;//195;
...
async function renderFile(buffer: ArrayBuffer, frameBuffer: ArrayBuffer) {
    ...
    while (!seenFrames.has(frameNumber)) {
        ...
        const thermalRef = detectThermalReference(
            medianSmoothed,
            radialSmoothed,
            prevThermalReference,
            120,
            160
        );
