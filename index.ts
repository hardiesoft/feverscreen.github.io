...
const minFrame: number = -1;//151;
const maxFrame: number = -1;//195;
...
export function extractFaceInfo(neck: {left: Point, right: Point}, body: Shape, radialSmoothed: Float32Array, maybeHasGlasses: boolean): FaceInfo | null {
...
const distanceSq2 = (a: [number, number], b: [number, number]): number => {
    const dX = a[0] - b[0];
    const dY = a[1] - b[1];
    return dX * dX + dY * dY;
};
...
function drawShapesIntoMask(shapes: Shape[], data: Uint8Array, bit: number, width: number = 120) {
    for (const shape of shapes) {
        for (const span of shape) {
            let i = span.x0;
            if (span.x0 >= span.x1) {
