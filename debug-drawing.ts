...
import {edgeBit, FaceInfo, getHottestSpotInBounds, motionBit, thresholdBit} from "./body-detection.js";
...
export function drawCurveFromPoints(pointsArray: Uint8Array, canvas: HTMLCanvasElement | CanvasRenderingContext2D) {
    let ctx;
...
export function drawCurve(shapes: Shape[], canvas: HTMLCanvasElement) {
    if (shapes.length) {
...
export function drawPoint(p: Point, canvas: HTMLCanvasElement, color = 'green', radius = 2) {
    const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
    ctx.beginPath();
    ctx.fillStyle = color;
...
export function drawConvexShape(convexHull: ConvexHull, frameNum: number, canvas: HTMLCanvasElement) {
    const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
    let o = 1;
...
export function drawShapes(shapes: Shape[], frameNum: number, canvas: HTMLCanvasElement, color:number = 0x33ff00ff) {
    const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
...
export function drawFace(face: FaceInfo | null, canvas: HTMLCanvasElement, threshold: number, smoothedImageData: Float32Array) {
...
export function drawRawShapes(shapes: RawShape[], frameNum: number, canvas: HTMLCanvasElement, color:number = 0x33ff00ff) {
    const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
    const width = canvas.width;
    const height = canvas.height;
    const img = ctx.getImageData(0, 0, width, height);
...
function drawImage(canvas: HTMLCanvasElement, canvas2: HTMLCanvasElement, data: Uint8Array, frameNumber: number): [number, number][] {
    const ctx = canvas.getContext('2d') as CanvasRenderingContext2D;
            ...
            for (const cluster of clusters.clusters) {
                if (cluster.length < 15) {
                    ...
                    for (const pointIndex of cluster) {
                        const point = pp[pointIndex] as [number, number];
