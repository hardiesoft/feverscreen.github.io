...
export function extractFaceInfo(body, radialSmoothed, canvas, maybeHasGlasses) {
...
export function getHottestSpotInBounds(face, threshold, width, height, imageData) {
    ...
    const idealCenter = add(forehead.top, scale(normalise(sub(forehead.bottom, forehead.top)), distance(forehead.bottom, forehead.top) * 0.8));
...
const convexHull = (points, print = false) => {
...
function monotoneConvexHull2D(points) {
    ...
    for (let i = 2; i < n; ++i) {
        ...
        let m = lower.length;
        while (m > 1 && pointIsLeftOfOrOnLine(points[lower[m - 2]], points[lower[m - 1]], p)) {
            m -= 1;
            lower.pop();
        }
        lower.push(idx);
        ...
        m = upper.length;
        while (m > 1 && pointIsLeftOfOrOnLine(points[upper[m - 2]], points[upper[m - 1]], p)) {
            m -= 1;
            upper.pop();
        }
        upper.push(idx);
...
async function renderFile(buffer) {
