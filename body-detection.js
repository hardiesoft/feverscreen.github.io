import { add, allNeighboursEqual, boundsForConvexHull, boundsForRawShape, closestPoint, convexHullForShape, distance, distanceSq, distanceSq2, drawRawShapesIntoMask, drawShapesIntoMask, faceArea, faceHasMovedOrChangedInSize, faceIntersectsThermalRef, faceIsFrontOn, getRawShapes, getSolidShapes, isNotCeilingHeat, joinShapes, largestShape, localDensity, magnitude, narrowestSlanted, normalise, perp, pointIsInQuad, pointIsLeftOfLine, rawShapeArea, scale, shapeArea, shapesOverlap, spanWidth, sub, widestSpan } from "./geom.js";
...
export function detectBody(sobel, thermalReference, medianSmoothed, radialSmoothed, prevRadialSmoothed, min, max, threshold, thermalRefC, thermalRefRaw) {
...
export function mergeHeadParts(shapes) {
    const mergedShapes = [];
    if (shapes.length) {
...
export function preprocessShapes(frameShapes) {
...
export function guessApproximateHeadWidth(body) {
    let last = body[0];
...
export function refineHeadThresholdData(data, neck, pointCloud) {
...
export function refineThresholdData(data) {
    const points = [];
    ...
    if (points.length > 10) {
        ...
        if (pointCloud.length > 15) {
            ...
            let first = hull.findIndex(([x, y]) => y === HEIGHT - 1);
...
export function extractFaceInfo(neck, faceShape, radialSmoothed, maybeHasGlasses) {
    ...
    if (heightProbeP) {
        ...
        if (isValidHead) {
            ...
            const foreheadTopP = add(neckBaseMiddleP, scale(normMidline, scaleFactor * 0.85));
            const foreheadBottomP = add(neckBaseMiddleP, scale(normMidline, scaleFactor * 0.75));
...
export function advanceScreeningState(nextState, prevState, currentCount) {
...
export function advanceState(prevMotionStats, motionStats, face, body, prevFace, screeningState, screeningStateCount, threshold, radialSmoothed, thermalReference) {
    let next;
    let event = "";
...
export const rotate90 = (src, dest) => {
    let i = 0;
    const width = 160;
    const height = 120;
    for (let y = 0; y < height; y++) {
